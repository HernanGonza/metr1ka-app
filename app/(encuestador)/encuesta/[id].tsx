    ...
    <View style={{ flex: 1, backgroundColor: "#f2f1ee" }}>
      ...
      <View style={mn.header}>
        <View style={mn.headerTop}>
          ...
          <Text style={mn.statsText}>
            {stats.completadas}/{stats.total_parcelas} encuestas
          </Text>
        </View>
        <Text style={mn.headerLabel}>Próxima visita</Text>
        <Text style={mn.headerDir}>
          {parcela?.direccion || "Calculando dirección..."}
        </Text>
        ...
        <View style={mn.statsRow}>
          <Text style={mn.statsText}>
            {stats.completadas}/{stats.total_parcelas}
          </Text>
      ...
      <View style={{ flex: 1, position: "relative" }}>
        {destLat && destLng ? (
          !MapLibreDisponible ? (
            <MapaPlaceholder mensaje="Mapa de navegación disponible en la app compilada" />
          ) : (
            <MLMap
              style={{ flex: 1 }}
            ...
            >
              <Camera
                initialViewState={{
                  center: [
                    ubicacion?.lng ?? destLng,
                    ubicacion?.lat ?? destLat,
                  ] as [number, number],
                  zoom: 15,
                }}
              />
        ...
        {enParcela && (
          <View style={mn.llegadaBadge}>
            ...
            <Text style={mn.llegadaSub}>
              Tocá la puerta y presioná "Comenzar"
            </Text>
          </View>
        )}
        ...
        {!enParcela && ubicacion && destLat && destLng && (
          <View style={mn.distBadge}>
            <Text style={mn.distText}>
              {Math.round(
                distanciaMetros(ubicacion.lat, ubicacion.lng, destLat, destLng),
              )}{" "}
              m
            </Text>
          </View>
      ...
      <View
        style={[mn.footer, { paddingBottom: Math.max(insets.bottom + 12, 16) }]}
      >
...
function PreguntaCard({
  ...
  const opciones = [...(pregunta.opciones_pregunta || [])].sort(
    ...
    >
      ...
      {pregunta.tipo === "si_no" && (
        <View style={{ gap: 10, marginTop: 8 }}>
          {["Sí", "No"].map((op) => (
            <TouchableOpacity
              key={op}
              style={[s.opcionBtn, respuesta === op && s.opcionSel]}
              onPress={() => onChange(op)}
            >
              <View
                style={[s.opcionCheck, respuesta === op && s.opcionCheckSel]}
              >
                {respuesta === op && (
                  <Text
              ...
              <Text style={[s.opcionText, respuesta === op && s.opcionTextSel]}>
                {op}
              </Text>
            </TouchableOpacity>
          ))}
        ...
        <View style={{ gap: 10, marginTop: 8 }}>
          {opciones.map((op: any) => {
            ...
            return (
              <TouchableOpacity
                key={op.id}
                style={[s.opcionBtn, selId === op.id && s.opcionSel]}
                onPress={() => onChange({ opcionId: op.id, texto: op.texto })}
              >
                <View
                  style={[s.opcionCheck, selId === op.id && s.opcionCheckSel]}
                >
                  {selId === op.id && (
                    <Text
                      style={{ color: "#fff", fontSize: 11, fontWeight: "800" }}
                ...
                <Text
                  style={[s.opcionText, selId === op.id && s.opcionTextSel]}
                >
                  {op.texto}
                </Text>
              </TouchableOpacity>
        ...
        <View style={{ marginTop: 12 }}>
          <View style={s.escalaGrid}>
            {Array.from({ length: 10 }, (_, i) => i + 1).map((n) => (
              <TouchableOpacity
                key={n}
                style={[s.escalaBtn, respuesta === n && s.escalaBtnSel]}
                onPress={() => onChange(n)}
              >
                <Text
                  style={[s.escalaText, respuesta === n && s.escalaTextSel]}
                >
                  {n}
                </Text>
              </TouchableOpacity>
      ...
      {pregunta.tipo === "matriz" &&
        (() => {
          ...
          const filas = (cfg.filas || []).map((f: any) =>
            typeof f === "string" ? f : f.texto,
          );
          const columnas = (cfg.columnas || []).map((c: any) =>
            typeof c === "string" ? c : c.texto,
          );
          if (!filas.length || !columnas.length) return null;
          const val: Record<number, string> = respuesta
            ? typeof respuesta === "string"
              ? JSON.parse(respuesta)
              : respuesta
            : {};
                ...
                {filas.map((fila, fi) => (
                  <View
                    ...
                    style={{
                      ...
                      backgroundColor:
                        fi % 2 === 0 ? "transparent" : "rgba(0,0,0,0.01)",
                      minHeight: 48,
                    }}
                  >
...
export default function EncuestaScreen() {
  ...
  const pantallaRef = useRef<Pantalla>("mapa");
  useEffect(() => {
  ...
  useEffect(() => {
    const sub = BackHandler.addEventListener("hardwareBackPress", () => {
      ...
      if (p === "mapa" || p === "fin") {
        // En el mapa o al final, salir normal
        return false;
      }
      // En medio de la encuesta, pedir confirmación
      Alert.alert(
        "¿Salir de la encuesta?",
        "Si salís ahora las respuestas de esta encuesta no se guardarán.",
        [
          { text: "Seguir encuestando", style: "cancel" },
          { text: "Salir", style: "destructive", onPress: () => router.back() },
        ],
      );
      return true; // bloquea el back por defecto
    });
    return () => sub.remove();
  }, []); // solo se registra una vez — usa pantallaRef para leer el valor actual
  ...
  const todasLasPreguntas = useMemo(
    () =>
      preguntas
        .filter((p) => p.clave_base !== "participa")
  ...
  function handleSiguiente() {
    ...
    if (result?.accion === "finalizar") {
      guardarYFinalizar();
      return;
    }
    if (result?.accion === "saltar" && result.destino_id) {
      const idx = preguntasVisibles.findIndex(
        (p: any) => p.id === result.destino_id,
      );
      if (idx >= 0) {
        setPaso(idx);
        return;
  ...
  async function guardarYFinalizar(razon?: string) {
    ...
    try {
      const filas = Object.entries(respuestas).map(([pregunta_id, valor]) => {
        ...
        const esMatriz =
          valor !== null && typeof valor === "object" && !esOpcion;
        return {
          pregunta_id,
  ...
  if (pantalla === "mapa") {
    ...
    if (!parcela)
      return (
        <View style={s.centered}>
          <Text style={{ fontSize: 48, marginBottom: 16 }}>🎉</Text>
          <Text style={[s.finTitle, { marginBottom: 12 }]}>
            ¡Zona completada!
          </Text>
          <Text style={s.finDesc}>
            No quedan más parcelas para visitar en esta zona.
          </Text>
          <TouchableOpacity style={s.btnComenzar} onPress={() => router.back()}>
            <Text style={s.btnComenzarText}>← Volver al inicio</Text>
          </TouchableOpacity>
      ...
      <View style={{ flex: 1 }}>
        <MapaNavegacion
          ...
          stats={{
            ...
            completadas: parcela.completadas,
          }}
        ...
        <ScrollView contentContainerStyle={{ padding: 24, paddingTop: 40 }}>
          ...
          <View style={{ gap: 10, marginTop: 8 }}>
            {razonesNR.map((r) => (
              <TouchableOpacity
                key={r}
                style={[s.opcionBtn, razonNR === r && s.opcionSel]}
                onPress={() => setRazonNR(r)}
              >
                <View
                  style={[s.opcionCheck, razonNR === r && s.opcionCheckSel]}
                >
                  {razonNR === r && (
                    <Text
                      style={{ color: "#fff", fontSize: 11, fontWeight: "800" }}
                ...
                <Text style={[s.opcionText, razonNR === r && s.opcionTextSel]}>
                  {r}
                </Text>
              </TouchableOpacity>
            ))}
  ...
  return (
    <View style={s.centered}>
      ...
      <Text style={s.finTitle}>
        {noResponde ? "Registrado" : "¡Encuesta completada!"}
      </Text>
      <Text style={s.finDesc}>
        {noResponde
